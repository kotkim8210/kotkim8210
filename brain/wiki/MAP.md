...
- 누적 통찰(insights): **7**
- 보관(archive): 0
- 실측 데이터셋: 3 (`brain/data/` — 경남·인천·서울) · 계산도구: `tools/순이익률_진단시트.xlsx`
- 마지막 수집(ingest): 2026-08-15 (서울 계약공개 돌파 + 단위경제 모델 신설)
...
- [[cleaning-unit-economics]] — ⭐**손익분기 = 매출/man-day 188,500원**. 이 한 줄로 B2C(적자권)와 학교 고액(60%대)이 갈린다. 경쟁강도·연간손익 포함. `#단위경제 #수익성 #실측`
...
school-cleaning-market ──(세그먼트 정의 제공)──► cleaning-unit-economics
   └ A형/B형 구분 → 손익분기 man-day로 수익성 계량
     💡 "마진을 깎는 것은 경쟁자가 아니라 투입 man-day다" (1인수의 시장이라 가격경쟁이 없음)

cleaning-unit-economics ══(B형 손익분기 105md)══► social-coop-procurement
   └ 연간 상주형은 인건비 보조 없이는 구조적 적자 → 사회적경제가 64% 장악하는 이유
...
- [✅돌파 2026-08-15] **서울 계약공개 접근 성공** (POST list0010d.do). 416기관×4개월 표본 수집. 단 공개범위가 '수의계약'이라 고액 경쟁계약 누락 가능성 — [[cleaning-unit-economics]] 참조.
- [확인필요] **경기는 여전히 차단**(schCntrNm·schCntrInstNm 모두 무시). 나라장터 API는 인증키 필요(401).
- [⚠️정정 2026-08-15] [[school-cleaning-market]] — 현업 증언(건당 3~4천만원)이 맞았고 이전 '돈 안 됨' 판정이 틀렸다. 중앙값이 아니라 상위 10% 구간을 봐야 했다.
- [🔴최우선] [[cleaning-unit-economics]] — **man-day가 전 모델을 지배하는데 실측이 없다.** 증언 5일 기준 이익률 85%, 15일이면 56%, 30일이면 11%. 동생분 확인 4개 항목으로 해소 가능.
- [🔴최우선] [[school-cleaning-market]] — 증언이 **A형(1회성)인지 B형(연간)인지 미판별**.
