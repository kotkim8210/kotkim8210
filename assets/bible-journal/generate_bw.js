// 1단계 POD용 흑백 본문 (흰 배경 + 검정 잉크). 표지(컬러)는 별도 파일.
...
const BG = '#FFFFFF';                 // 종이(인쇄 안 함) → 흰 배경
const INK = '#141414';                // 검정 잉크 (흑백)
const INK_SOFT = 'rgba(20,20,20,0.60)';
const INK_FAINT = 'rgba(20,20,20,0.26)';
const INK_RULE = 'rgba(20,20,20,0.34)';

function page() { const c = createCanvas(W, H); const x = c.getContext('2d'); x.fillStyle = BG; x.fillRect(0, 0, W, H); return { c, x }; }
...
function rChildren() { const { c, x } = page(); center(x, '우리 아이들', W / 2, 410, 66, 'NMX', INK); rule(x, W / 2 - 80, 478, W / 2 + 80, INK_FAINT, 2); let y = 640; for (let i = 0; i < 3; i++) { left(x, '이름', M, y, 42, 'NMB', INK); rule(x, M + 150, y + 8, M + 620, INK_RULE, 2); left(x, '생일', M + 700, y, 42, 'NMB', INK); rule(x, M + 850, y + 8, W - M, INK_RULE, 2); left(x, '기도 키워드', M, y + 120, 38, 'NMR', INK_SOFT); rule(x, M, y + 200, W - M, INK_FAINT, 1.6); rule(x, M, y + 290, W - M, INK_FAINT, 1.6); y += 430; } return c; }
function rSection(title) { const { c, x } = page(); frame(x, 90); center(x, title, W / 2, 1240, fit(x, title, W - M * 2 - 100, 110, 'NMX'), 'NMX', INK); rule(x, W / 2 - 110, 1330, W / 2 + 110, INK_SOFT, 2.5); sprig(x, W / 2, 1620, 1.5, INK); return c; }
function rDaily() { const { c, x } = page(); left(x, '날짜', M, 250, 44, 'NMB', INK); rule(x, M + 110, 258, M + 620, INK_RULE, 2); x.textAlign = 'right'; x.font = '30px "NMR"'; x.fillStyle = INK_SOFT; x.fillText('DAILY  PRAYER', W - M, 250); x.textAlign = 'left'; rule(x, M, 300, W - M, INK_FAINT, 1.5); function sec(label, y, lines, gap = 96) { left(x, label, M, y, 46, 'NMB', INK); for (let i = 0; i < lines; i++) rule(x, M, y + 70 + i * gap, W - M, INK_FAINT, 1.6); return y + 70 + lines * gap; } let y = 430; y = sec('오늘의 말씀', y, 3) + 70; y = sec('자녀를 위한 기도제목', y, 4) + 70; y = sec('기도 응답 기록', y, 3) + 70; y = sec('오늘의 감사', y, 2) + 40; sprig(x, W - M - 30, H - 150, 1.0, INK); return c; }
function rLog() { const { c, x } = page(); center(x, '기도 응답 기록', W / 2, 280, 78, 'NMX', INK); const top = 470, bot = H - 230, l = M, r = W - M, c1 = l + 230, c2 = l + 760, hH = 110; x.strokeStyle = INK_SOFT; x.lineWidth = 2.5; x.strokeRect(l, top, r - l, bot - top); rule(x, l, top + hH, r, INK_SOFT, 2.5); center(x, '날짜', (l + c1) / 2, top + 70, 40, 'NMB', INK); center(x, '기도제목', (c1 + c2) / 2, top + 70, 40, 'NMB', INK); center(x, '응답하심', (c2 + r) / 2, top + 70, 40, 'NMB', INK); vline(x, c1, top, bot, INK_FAINT, 1.5); vline(x, c2, top, bot, INK_FAINT, 1.5); const rows = 11, rh = (bot - (top + hH)) / rows; for (let i = 1; i < rows; i++) rule(x, l, top + hH + i * rh, r, INK_FAINT, 1.4); sprig(x, W / 2, H - 130, .8, INK); return c; }
function rReading(title, books, cols) { const { c, x } = page(); center(x, title, W / 2, 280, 70, 'NMX', INK); const top = 470, colW = (W - M * 2) / cols, rows = Math.ceil(books.length / cols), rh = Math.min(118, (H - top - 230) / rows); for (let i = 0; i < books.length; i++) { const col = Math.floor(i / rows), row = i % rows; const bx = M + col * colW, by = top + row * rh; checkbox(x, bx, by, 34); left(x, books[i], bx + 56, by + 30, 34, 'NMR', INK); } sprig(x, W / 2, H - 130, .7, INK); return c; }
function rReflection() { const { c, x } = page(); center(x, '한 해의 기도를 돌아보며', W / 2, 420, 60, 'NMX', INK); rule(x, W / 2 - 90, 490, W / 2 + 90, INK_FAINT, 2); const blocks = ['올해 가장 큰 기도 응답', '내 마음에 일어난 변화', '내년, 자녀를 위한 새 기도']; let y = 660; for (const b of blocks) { left(x, b, M, y, 46, 'NMB', INK); for (let i = 0; i < 3; i++) rule(x, M, y + 80 + i * 96, W - M, INK_FAINT, 1.6); y += 480; } sprig(x, W - M - 30, H - 150, 1.0, INK); return c; }
...
// 본문(표지 제외) 110쪽 = 짝수, 무선/와이어 모두 적합
const DAILY = 93, LOG = 7;
const order = [];
order.push(rTitle(), rHowTo(), rOwner(), rChildren(), rSection('자녀를 위한 기도'));
...
const out = fs.createWriteStream(`${DIR}/엄마의_기도_노트_본문_흑백_110p.pdf`);
doc.pipe(out);
...
doc.end();
out.on('finish', () => console.log('DONE 본문 흑백 페이지수 = ' + order.length));

// 미리보기 PNG
fs.writeFileSync(`${DIR}/bw_title.png`, rTitle().toBuffer('image/png'));
fs.writeFileSync(`${DIR}/bw_daily.png`, rDaily().toBuffer('image/png'));
fs.writeFileSync(`${DIR}/bw_reading.png`, rReading('성경 통독표 · 구약', OT, 3).toBuffer('image/png'));
