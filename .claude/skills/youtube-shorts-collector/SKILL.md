...
> **채팅 보고 규칙**: 수집 결과를 사용자에게 요약해 보여줄 때, 영상을 언급하는 모든 곳(표·목록·추천·비교)에 **URL을 반드시 병기**한다. 제목만 쓰지 않는다 — 사용자가 따로 검색하지 않고 바로 클릭해 볼 수 있어야 한다. 60초 이하 쇼츠는 `youtube.com/shorts/ID` 형식이 바로 쇼츠 플레이어로 열려 더 좋다.
