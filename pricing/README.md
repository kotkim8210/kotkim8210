...
- **공급가**: 중품 −18~29% 폭락(시즌 종료). 특품·애플 거의 보합. PBF 어드민 2번째 스냅샷 반영.
- **시세**: WebSearch 집계로 중품·특품 실측치 확보(sample=false). 애플은 직접 관측 실패 → 특품×1.2 추정(sample=true).
- **등록가**: 옥수수 12개 옵션 모두 시세 + 산지직송 프리미엄(중품 +1,000원 / 특품 +2,000원 / 애플 +2,000원)으로 조정 완료.
- **수수료**: 당근 공식 안내인 **통합 3.3%(결제+판매, VAT 포함)** 로 확정. 별도 결제수수료 없음.
- **마진**: 중품 시세 기준 ~28~39%, 특품 ~8~14%, 애플 ~14~26%. 특품 15개만 ⚠️ 5% 미만(보수적 시세 추정 영향).
