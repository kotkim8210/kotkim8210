...
- 조각 풀 19종 → 33종 → **42종** (발주자 "다양성" 요청, 2026-07-11): T 4방향 전부, S/Z 가로·세로, **L/J 테트로미노 8방향 전부**, 3칸·5칸 코너 각 4방향, 3×2 직사각형, **십자(+) 5칸**, **대각선 2·3칸 4종**(Woodoku형). 소형 조각 가중치 상향(dot 4→7, i2/v2 8→9)과 대각선 저가중치로 가중 평균 조각 크기 **3.79셀** (§10 목표 3.78 대비 +0.3%). 데일리 시퀀스는 시드 결정성 유지(풀 확장으로 기존 시퀀스와는 달라짐 — 출시 전이므로 영향 없음).
- ui-spec §6 "색+형태 이중 구분(조각별 미세 패턴 점)" — **구현 완료** (2026-07-11): 젬 색상별 6종 마이크로 도트 글리프(발자국·별·하트 배열, `.g0`~`.g5`)를 보드 셀·트레이·드래그 클론에 적용. 이전 "남은 리스크" 항목 해소.
- **웜스타트 보드** (발주자 "초반 클리어 희소" 피드백, 2026-07-11): 매 판 하단 6~8행에 10~21셀을 사전 배치(행당 최대 7칸·열당 최대 3칸·완성 줄 0 보장, `warmstart.ts`)해 첫 클리어 도달을 ~15수 → 2~4수로 단축. 클래식은 판 시드, 데일리는 날짜 시드에서 결정적으로 생성(전 세계 동일 지형 유지). 플레이어 회전 기능은 PRD Won't 유지(모든 방향은 42종 스폰 풀로 제공).
